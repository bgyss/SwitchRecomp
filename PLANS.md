...
Work items
- Add a standalone policy document that covers legal acquisition, asset separation, and prohibited content.
- Add a short policy summary to `README.md` and link the policy doc from `RESEARCH.md` and `ROADMAP.md`.
- Add a tooling guardrail note describing how provenance requirements are enforced (ties into SPEC-020).
...
Work items
- Formalize the baseline profile as a structured document or config (CPU, GPU, memory, timing modes).
- Add a runtime configuration stub that can switch between handheld and docked timing modes.
- Record platform assumptions and trace which specs depend on them.
...
Work items
- Define a provenance metadata schema (TOML or JSON) and add a validator.
- Add CLI support that refuses to build without valid provenance metadata.
- Implement format detection for NCA, ExeFS (PFS0), NSO0, NRO0, NRR0, and NPDM inputs.
- Add non-proprietary test fixtures and tests that prove format detection and provenance logging.
...
Work items
- Define a packaging layout spec (code vs assets separation) and include it in docs.
- Add a reference packaging command to the CLI or a build script.
- Create a release checklist template that includes legal compliance checks.
- Add tests that verify build manifest checksums match the emitted bundle contents.
...
Work items
- Define the baseline test suite and target thresholds for correctness and stability.
- Add a regression report generator (for example, JSON summary + human-readable output).
- Add CI wiring that runs the baseline suite and fails on regressions.
- Document how to add new golden traces without distributing proprietary assets.
...
Work items
- Create a shortlist of 2 to 3 candidate titles and document pros/cons.
- Produce a service dependency map and estimated instruction coverage for each candidate.
- Write a final selection memo and a private trace-collection plan.
